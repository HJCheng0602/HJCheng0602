...
function cube(x, y, z, i, delay) {
  const [top, right, left] = colors[i % colors.length], s = 11, h = z * 12;
  return `<g class="cube c${delay % 8}" transform="translate(${x} ${y-h})"><polygon points="0,0 ${s},${s/2} 0,${s} -${s},${s/2}" fill="${top}" stroke="#b6d9ff" stroke-opacity=".18" stroke-width=".45"/><polygon points="${s},${s/2} ${s},${s/2+12} 0,${s+12} 0,${s}" fill="${right}"/><polygon points="0,${s} 0,${s+12} -${s},${s/2+12} -${s},${s/2}" fill="${left}"/></g>`;
}
function terrain() {
  ...
  const ridge = days.map((day, i) => {
    const h = Math.max(1, Math.round(10 * (day.tokens || 0) / max)), x = 146 + i * 21, y = 151 + i * 12;
  ...
  const labels = days.filter((_, i) => i % 5 === 0 || i === days.length - 1).map((day, i) => `<text x="${132 + (i * 5) * 21}" y="421" class="axis">${day.date.slice(5)}</text>`).join('');
  return ridge + labels;
}
function rows() {
